...
angular.module('geckoApp').service('TreemapDataService', ['$http', '$rootScope', 'apiUrl', function ($http, $rootScope, apiUrl) {
    var currentDate,
        onloadCbArr = [],
        currentTreemapData = {
            name: 'Treemap',
    ...
    $http.get(apiUrl + '/map/').success(function(data, status, headers, config) {
    ...
    $http.get(apiUrl + '/dates').success(function(dates, status, headers, config) {
        $rootScope.data.datesArr = dates;
        $rootScope.data.validDates = {};
        $rootScope.data.currentDate = currentDate = $rootScope.data.datesArr[0];
        $http.get(apiUrl + '/weights/' + currentDate).success(function(data, status2, headers2, config2) {
            //process data
    ...
    var setDate = function (date, cb) {
        ...
        if ($rootScope.data.weightsDB[currentDate]) {
            ...
            $http.get(apiUrl + '/weights/' + currentDate).success(function(data, status, headers, config) {
                $rootScope.data.weightsDB[currentDate] = data;
                update(data);
                cb(null, currentTreemapData);
            });
